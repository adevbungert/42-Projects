...
/*   Updated: 2016/04/27 13:15:35 by abungert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		check_swap_beg(t_pushswap *data)
{
	...
	{
		if (data->pile_a[i - 1] < data->pile_a[i] || data->pile_a[i - 1] > data->pile_a[1])
			return ;
		i++;
	}
	rotate_a(data);
...
void	sort_pile(t_pushswap *data)
{
	while (42)
	{
		if (data->length_a == 3 && check_pile_reverse(data->pile_a, data->length_a))
		{
			reverse_rotate_a(data);
			swap_a(data);
		}
